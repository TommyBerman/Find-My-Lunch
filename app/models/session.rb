class Session < ApplicationRecord
    ...
    def self.select_array
        ...
        if @final_filter.count > 0
            $notice = "Here's something you might like!"
            @final_filter
        elsif @tags_filter.count > 0
            $notice = "Sorry! We couldn't find anything in your price range, but here's something nearby that meets your cravings!"
            @tags_filter
        else
            $notice = "Sorry! We couldn't find anything that meets all your requirements, but here is something nearby that you might like!"
            @location_filter
        end
    end
